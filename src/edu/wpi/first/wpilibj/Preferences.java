...
public class Preferences {
                    ...
                    synchronized (lock) {
                        if (!ImproperPreferenceKeyException.isAcceptable(key) || value.toString().indexOf('"') != -1) {
                            ...
                            for(int i = 0; i<prefListeners.size(); ++i)
                                ((ITableListener)prefListeners.get(i)).valueChanged(null, key, value.toString(), isNew);
                        }
                    }
                }
            }
    ...
    public void addPrefListener(ITableListener listener)
    {
        prefListeners.add(listener);
    }
    
    public boolean removePrefListener(ITableListener listener)
    {
        return prefListeners.remove(listener);
    }
